...
<div class="col-xl-8 col-lg-8 col-md-9 col-11">
    ...
    <div class="card shadow-5-strong">
        ...
        <form action="{{route('Mahasiswa.update',$data->id)}}" method="POST" class="card-body form-horizontal p-4">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="Nim" class="form-control-label">Nim</label>
                <input type="number" id="Nim" value="{{$data->nim}}" name="nim" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="Name" class="form-control-label">Name</label>
                <input type="text" id="Names" value="{{$data->names}}" name="names" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="Born" class="form-control-label">Born</label>
                <input type="date" id="Born" name="born" value="{{$data->born}}" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="Majors" class="form-control-label">Majors</label>
                <select name="majors" id="Majors" class="form-select" required>
                    <option disabled selected>{{$data->majors}}</option>
